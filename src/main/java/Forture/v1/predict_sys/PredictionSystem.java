...
public class PredictionSystem extends QuoteSystem implements ActionListener
{
    ...
    private double forecast = Double.MIN_VALUE;

    private HashMap<String, Double> forecasts;
    ...
    public String getQuote( String symbol )
    {
        ...
        double firQrtl = 0.0;
        double thirQrtl = 0.0;
        AlphaVantageConnector endpointConnection = new AlphaVantageConnector( StockTool.API_KEY,
            iLoveYou );
        TimeSeries stockTimeSeries = new TimeSeries( endpointConnection );
        ...
        try
        {
            ...
            {
                if ( Math.abs( deriv.get( i ) ) <= threshold )
                {
        ...
        int maxIndex )
    {
        ...
        rating = new ArticleRating( symbol ).getRating();
        ...
        {
            return ( ( ( 1 + ( ( arocMid - arocEnd ) / ( arocEnd * ( midIndex - endIndex ) ) ) ) )
                * ( rating ) ) + list.get( 0 ).getClose();
            // return ( ( rating ) * ( max - min )
            // * ( 1 + ( ( arocMid - arocEnd ) / ( arocEnd * (endIndex -
            // midIndex) ) ) ) ) + list.get( 0 ).getClose();
    ...
    /**
     * If it is makegraph, then get the graph. Else if it is getquote, then get
     * the data.
    ...
    public void processGraph()
    {
        if ( action == 2 )
        {
                                                                                                    ...
                                                                                                    // symbol
            {
                ...
                // new ArticleRating(stockTool.translateToName(textField.getText().toString().toLowerCase()).toUpperCase()).getRating()
                rating = ( rating == -2 ? -0.078 : rating );

                graph = new PredictionGraph( textField.getText().toString().toLowerCase(),
                    forecast );
